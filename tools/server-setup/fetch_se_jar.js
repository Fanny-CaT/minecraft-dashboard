...
async function main() {
  ...
  const res = await fetch(`${ROOT}/proxy/daemon/server/${ID}/file/plugins/SuperEnchants-4.6.2.jar`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  const arrayBuffer = await res.arrayBuffer();
  fs.writeFileSync('tools/server-setup/SuperEnchants.jar', Buffer.from(arrayBuffer));
}
