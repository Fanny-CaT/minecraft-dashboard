...
async function getLatestModrinth(project) {
  const res = await fetch(`https://api.modrinth.com/v2/project/${project}/version`, {
...
async function main() {
  ...
  try {
    const data = await getLatestModrinth('excellentenchants');
    if (data) {
      await uploadFile(token, `plugins/${data.filename}`, data.buffer);
    } else {
      console.log(`Failed to find excellentenchants on Modrinth`);
    }
