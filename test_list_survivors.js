...
async function run() {
  ...
  let res = await fetch(`${url}/plugins/Essentials`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  let data = await res.json();
  console.log("Essentials:", JSON.stringify(data, null, 2));

  res = await fetch(`${url}/plugins/SkinsRestorer`, {
    headers: { Authorization: `Bearer ${token}` }
  });
  data = await res.json();
  console.log("SkinsRestorer:", JSON.stringify(data, null, 2));
}
run();
